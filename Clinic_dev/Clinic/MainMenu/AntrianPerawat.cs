...
namespace Clinic
{
    public partial class AntrianPerawat : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void loadAntrianNo()
        {
            try
            {
                ...
                dtAntrian = ConnOra.Data_Table_ora(sql);
            }
            catch { }
        ...
        private void ListAntrianNm()
        {
            try
            {
                ...
                dtAntrianNm = ConnOra.Data_Table_ora(sql);
            }
            catch
            {
            }
        ...
        private void loadGridAntrian(string codePoli)
        {
            try
            {
                ...
                if(codePoli.ToString().Equals("REG"))
                {
                     sql = @"      SELECT ROWNUM AS C_NO, A.* FROM (
                                         ...
                                         WHERE     1 = 1
                                               ...
                                               AND QUE not in (select que01 from  cs_visit b where to_char(visit_date,'yyyy-mm-dd')= to_char(sysdate, 'yyyy-mm-dd') ) 
                                        ORDER BY INS_DATE
                                       ) A"; 
                }
                else
                {
                     sql = @"      SELECT ROWNUM AS C_NO, A.* FROM (
                                            SELECT A.QUE01  NO_PASIEN, 
                                               B.NAME       NAMA_PASIEN,
                                               A.PURPOSE
                                          FROM KLINIK.TBL_KUNJUNGAN A, KLINIK.CS_PATIENT_INFO B
                                         WHERE     1 = 1
                ...
                dtGridAntrian = ConnOra.Data_Table_ora(sql);
        ...
        private void loadGridHold(string codePoli)
        {
            try
            {
                ...
                dtGridHold = ConnOra.Data_Table_ora(sql);
